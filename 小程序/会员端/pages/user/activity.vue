			...
			<view class="contentPanel" style="border-radius: 18rpx; margin-top: 20rpx;" v-for="(item,index) in clubActivityList" :key="index" @click="clickToActivityDetail(item)">
				<view class="booksList">
					<view class="books-info">
						...
						<view class="info">
							<view class="f26 ellipsis" style="font-weight: bold;">{{item.clubActivityName}}</view>
							<view class="f26 line40">报名时间：{{item.createTimeText | toTime}}</view>
							<view class="f26 ellipsis">地点：{{item.clubActivityAddress | toNull}}</view>
							<view class="f26 line40">剩余次数：{{item.surplusQuantity}}</view>
	...
	export default {
		...
		methods: {
			bindData(){
				...
				// data.memberId = common.getMemberId()
				// data.fields ="*,DATE_FORMAT(expiry_date, '%Y年%m月%d日') AS formatted_expiry_date,IFNULL((SELECT surplus_quantity from club_activity_member where member_id="+common.getMemberId()+"  and club_activity_id=view_club_activity.id),0) as surplus_quantity"
				//data.condition = "id in (SELECT club_activity_id from club_activity_member where member_id="+common.getMemberId()+")" 
				data.pageIndex = that.pageIndex
				data.pageSize = that.pageSize 
				data.orderBy = 'create_time desc';
				common.call("GetViewClubActivityMemberList", data, function(result) {
					console.log(result)
