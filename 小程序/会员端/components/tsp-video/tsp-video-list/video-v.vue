					...
					<view :style="{opacity:shadeNum > 2 ?'0':'1'}">
						<view class="vodPlayer">
							...
							:muted="index == vodIndex ? false : true" 
							 :id="'myVideo'+index" @play="startPlay(index)" @waiting="bufferVod(index)" @timeupdate="timeupdateVod($event,index,item.isPay)" @error="errVod(index)" @ended="endedVod(index)">
							</video>
						</view>
						<!-- 视频封面 -->
						...
						<view class="vodPause-img" v-if="item.pauseShow">
							<image src="../../../static/image/video/vodPause.png" class="vodPauseImage"></image>
						</view>
						<view @click="playSpot(index,item.isPay)" class="vodPlayView" :style="videoStyle" @touchstart="vodViewStart($event)"
						...
						<view>
							<videoMenu :ref="'menuRef'+index" :vodIndex="vodIndex" :vodCurIndex="vodCurIndex" :item="item" :discussNum="totalPlayList.length" :index="index"
							 :sliderDrag="sliderDrag" :moveOpacity="moveOpacity" :palyCartoon="palyCartoon" @handleInfo="handleInfo" @handleOneFeeInfo="handleOneFeeInfo" @handlePayInfo="handlePayInfo" @handlePay="handlePay"></videoMenu>
						</view>
					</view>
					<!-- 底层图标 -->
					<!-- <view class="vodLayer">
...
<script>
	...
	export default {
		...
		methods:{
			// handleInfo(e){
			// 	console.log("回调")
			// 	this.$emit('handleInfo',e)  //点赞成功
			// },
			// handleOneFeeInfo(e){
			// 	console.log("一元支付回调")
			// 	this.$emit('handleOneFeeInfo',e)  //一元支付回调
			// },
			// handlePayInfo(){
			// 	console.log("所有视频能看回调")
			// 	this.$emit('handlePayInfo')  //三天免费  19.9支付 365支付  所有视频能看
			// },
			handlePay(e){
				this.$emit('handlePay',e)  //点击付费
			}
		}
	}
</script>
