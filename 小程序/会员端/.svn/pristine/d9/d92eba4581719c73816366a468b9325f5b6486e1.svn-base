...
function checkLogin(returnUrl) {
	var loginName = uni.getStorageSync('memberId');
...
function formatTable(content){
	var tableIndex = content.toLowerCase().indexOf("<table");
	while(tableIndex >= 0){
		var styleIndex = content.toLowerCase().indexOf("style=\"", tableIndex + 6);
		if(styleIndex >=0){
			var startIndex = styleIndex + "style=\"".length;
			var endIndex = content.toLowerCase().indexOf("\"", startIndex);
			var styleContent = content.substr(startIndex, endIndex - startIndex);
			content = content.substr(0, startIndex) + "max-width:100%" + content.substr(endIndex);
			// console.log(styleContent);
			// console.log(content);
			
		}
		tableIndex = content.toLowerCase().indexOf("<table", tableIndex + 6);
	}
	return content
}
...
module.exports.getCommunityId = getCommunityId;
