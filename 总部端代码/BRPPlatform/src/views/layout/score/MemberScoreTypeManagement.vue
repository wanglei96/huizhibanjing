...
import MemberScoreTypeDetail from './MemberScoreTypeDetail.vue'
...
function switchChange(value, row) {
  ...
  common.call("UpdateMemberScoreType", params, function (result) {
    list.loadData
  })
}
</script>
<template>
  <div class="basic-list">
    <MemberScoreTypeDetail ref="refDetail" @reload-data="list.loadData" />
          ...
          <template #default="scope">
            <el-space spacer="|" style="color: #999" wrap>
              <el-link type="warning" :id="scope.row.id"
                @click="list.openDetailDialog({ mode: 'view', primaryKeyValue: scope.row.code })">查看</el-link>
              <el-link type="primary" :id="scope.row.id"
                @click="list.openDetailDialog({ mode: 'edit', primaryKeyValue: scope.row.code })">编辑</el-link>
