...
function onSubmit() {
  ruleFormRef.value.validate((valid) => {
    if (valid) {
      ...
      data.sysUserId = common.getLocalStorage("sysUserId");
      common.call("ChangePassword", dataEntity, function (result) {
...
<template>
  <div>
    <el-dialog :close-on-click-modal="false" v-model="dialogFormVisible" title="修改密码" style="width: 450px">
      <el-form ref="ruleFormRef" :model="dataEntity" :rules="rules">
        <el-form-item label="旧密码" prop="oldPassword" :label-width="formLabelWidth">
          <el-input autocomplete="off" type="password" v-model="dataEntity.oldPassword" placeholder="请输入旧密码"/>
        </el-form-item>
        <el-form-item label="新密码" prop="password" :label-width="formLabelWidth">
          <el-input autocomplete="off" type="password" v-model="dataEntity.password" placeholder="请输入新密码"/>
        </el-form-item>
        <el-form-item label="确认密码" prop="rePassword" :label-width="formLabelWidth">
          <el-input autocomplete="off" type="password" v-model="dataEntity.rePassword"
