...
import SaveSysUserRoles from './SaveSysUserRoles.vue';
...
//管理员详情页组件引用
const refSysUserDetail = ref();
//设置角色组件引用
const refSaveSysUserRoles = ref();
let data = {
...
const list = management({
  ...
  treeTableName: "sys_organization",
  refDetail: refSysUserDetail,
  treeCondition: "is_deleted is not true",
  condition: "login_name !='admin' and is_deleted is not true"
})
const updateEnabled = (id, enabled) => {
  common.call('SetSysUserEnabled', { id, enabled });
}
...
const handleNodeClick = (val) => {
  ...
  list.parameters.sysOrganizationId = val.id;
...
<template>
  <div>
    <SysUserDetail ref="refSysUserDetail" @reload-data="list.loadData" />
    <SaveSysUserRoles ref="refSaveSysUserRoles" />
      ...
      <div class="dept-container">
          ...
          <div class="dept-aside-menu" :style="{ width: isShow ? '265px' : '0px' }">
            ...
            <el-scrollbar>
              <el-tree :data="list.treeData.value" :highlight-current="true" default-expand-all
                :expand-on-click-node="false" :props="list.defaultProps.value" node-key="id"
        ...
        <div class="dept-content">
              ...
              <el-row :gutter="20" type="flex">
                ...
                <el-col :xs="24" :sm="12" :md="8" :lg="6">
                  <el-form-item label="登录账号" prop="loginName">
                    <el-input placeholder="请输入..." clearable v-model="list.parameters.loginName"
                      @keydown.enter.prevent=list.query></el-input>
                  </el-form-item>
                </el-col>
                <el-col :xs="24" :sm="12" :md="8" :lg="6">
                  <el-form-item label="正式姓名" prop="trueName">
                    <el-input placeholder="请输入..." clearable v-model="list.parameters.trueName"
                      @keydown.enter.prevent=list.query></el-input>
                  </el-form-item>
                </el-col>
                <el-col :xs="24" :sm="12" :md="8" :lg="6">
          ...
          <el-card shadow="never" class="grid-card">
            ...
            <el-table ref="multipleTableRef" :data="list.dataList.value" border :table-layout="'auto'"
              style="width: 100%">
              <el-table-column property="employeeNo" label="员工编号" width="180" />
              <el-table-column property="loginName" label="登录账号" />
              <el-table-column property="trueName" label="员工姓名" />
              <el-table-column property="sysOrganizationName" label="部门" />
              <el-table-column property="enabled" label="是否启用">
                <template #default="scope">
                  <el-switch v-model="scope.row.enabled" @change="updateEnabled(scope.row.id, scope.row.enabled)" />
                </template>
              </el-table-column>
              <el-table-column label="操作" align="center" width="240">
                <template #default="scope">
                  <el-space spacer="|" style="color: #999" wrap>
                    ...
                    <el-link type="success" @click="refSaveSysUserRoles.getSysRoleListBySysUserId(scope.row.id)">
                      设置角色
                    </el-link>
                  </el-space>
                </template>
