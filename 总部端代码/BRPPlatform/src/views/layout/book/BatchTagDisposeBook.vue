...
function init(data) {
  ...
  list.value = management({
    ...
    condition: data.type == 'add' ? `company_id is null and id NOT IN (SELECT book_sku_id FROM book_sku_book_tag WHERE book_tag_id = ${data.item.id})` : `company_id is null and  id IN (SELECT book_sku_id FROM book_sku_book_tag WHERE book_tag_id = ${data.item.id})`,
    pageSize: 8,
