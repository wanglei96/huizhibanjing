...
const submitForm = async (formRef) => {
  ...
  await formRef.validate((valid, fields) => {
    if (valid) {
      ...
      common.call("CancelledMemberBorrowBook", data, function (result) {
...
<template>
  <div>
    <el-dialog destroy-on-close :close-on-click-modal="false" v-model="dialogFormVisible" title="取消用户所借书籍" :visible.sync="dialogFormVisible" width="600" draggable>
      <div style="color: red;margin-bottom: 20px;">该功能可以取消所有已借书籍</div>
      <el-form :model="dataItem" ref="formRef" :rules="rules" :label-position="'right'" label-width="150px">
        ...
        <el-form-item label="需要取消的图书书号" prop="bookSn">
