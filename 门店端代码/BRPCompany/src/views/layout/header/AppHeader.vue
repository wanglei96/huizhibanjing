...
// 导入修改密码对话框组件
import ChangePassword from '@/views/layout/system/ChangePassword.vue'
...
const companyUserName = ref(
  `${common.getLocalStorage('companyUserName') || ''}`
);
const companyName = ref(common.getLocalStorage('companyName') || '');
const companyNo = ref(common.getLocalStorage('companyNo') || '');

const getCompanyInfo = () => {
  if (companyNo.value && companyName.value) {
    return;
  } else {
    let params = {
      id: common.getLocalStorage('companyId')
    };
    common.call("GetCompany", params, (res) => {
      companyNo.value = res.data.companyNo;
      companyName.value = res.data.companyName;

      // 更新缓存数据
      common.setLocalStorage('companyName', res.data.companyName);
      common.setLocalStorage('companyNo', res.data.companyNo);
    });
  }
}
...
<template>
  <!-- 修改密码组件 -->
  <ChangePassword ref="dialogChangePassword" />
  <el-header :style="{ backgroundColor: styleForm.header.bgColor, color: styleForm.header.txtColor }">
    ...
    $t(item.meta.title)
        ...
        <router-link v-else :to="item.path" :style="{ color: styleForm.header.txtColor }">{{ $t(item.meta.title) }}
        </router-link>
      </el-breadcrumb-item>
    </el-breadcrumb>
      ...
      <!--      <el-popover placement="bottom" :width="320" trigger="click">
            ...
            <div class="msg-body">
              <div class="msg-item">
                ...
                <div class="msg-content">
                  <div class="msg-title">elementAdmin 2023年升级公告</div>
        ...
        <div class="bottom-tool">
          ...
          <span>
            <el-button
              text
              @click="saveTab({ name: '站内消息', path: '/personal/message/notice' })"
            >
              查看更多
            </el-button>
          </span>
    ...
    companyUserName
      ...
      <template #dropdown>
        <el-dropdown-menu>
          ...
          <el-dropdown-item @click="dialogChangePassword.init(1)">
            <el-icon>
              <Edit />
            </el-icon>
            {{ $t('修改密码') }}
  ...
  <el-drawer v-model="drawerVisible" size="300px">
    ...
    <div class="right-drawer">
      <el-form :model="styleForm" :inline="true">
        ...
        <el-row>
          <el-col :span="24">
            <el-button type="default" style="width: 100%" @click="handleReset">{{ $t('恢复默认') }}
            </el-button>
          </el-col>
        </el-row>
      </el-form>
