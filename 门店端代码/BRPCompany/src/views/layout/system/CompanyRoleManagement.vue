<script setup>
import {ref} from 'vue'
import CompanyRoleDetail from './CompanyRoleDetail.vue'
...
const list = management({
  ...
  getListServiceName: 'GetCompanyRoleList',
  condition: `name<>'超级管理员'`,
  refDetail
})

function deleteRow(id, name) {
  if (name !== '配送员' && name !== '拣书员') {
    list.deleteRow(id);
  } else {
    ElMessage({type: 'warning', message: '不能删除' + name + '角色'});
  }
}
</script>
<template>
  <div class="basic-list">
    <CompanyRoleDetail ref="refDetail" @reload-data="list.loadData"/>
        ...
        <el-table-column label="操作" align="center" width="240">
          <template #default="scope">
            <el-space spacer="|" style="color: #999" wrap>
              ...
              <el-link type="danger" @click="deleteRow(scope.row.id,scope.row.name)">删除</el-link>
              <el-link
                  type="success"
                  @click="refSetRolePermission.getRolePermissionTree(scope.row.id)"
              >设置权限
              </el-link>
            </el-space>
          </template>
