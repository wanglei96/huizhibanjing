...
const init = (options) => {
  ...
  //selectedRowKeys.value = selectedList.map(item => item.id)
  list.value = management({
    tableName: 'view_member',
    orderBy: 'create_time desc',
    getListServiceName: 'GetViewMemberList',
    condition:"member_card_card_no is not null",
    pageSize: 5
  })
...
function submit() {
  //获取所有选中的行
  emit('reload-data', { type: 'member', data: currentRow.value });
...
<template>
  <div class="basic-list">
    <el-dialog destroy-on-close :close-on-click-modal="false" v-model="detailFormVisible" title="添加会员" width="800"
      draggable>
      ...
      <el-card shadow="never">
        <el-alert v-if="mode !== 'Radio'" :title="`已选择 ${selectedColumnCount} 项`" v-show="selectedColumnCount > 0" type="warning" show-icon />
        <el-table ref="multipleTableRef" :data="list.dataList" style="width: 100%" border @selection-change="selectRow"
          :default-selection="selectedRowKeys" @current-change="handleCurrentChange" :row-key="row => row.id">
          ...
          <el-table-column property="name" sortable label="会员名称" />
          <el-table-column property="memberCardCardNo" sortable label="会员卡号" />
          <el-table-column property="mobile_phone" sortable label="手机号" align="center" />
        </el-table>
