...
function init(data) {
  ...
  if (data.type == 'delete') {
    title.value = '批量删除【' + data.item.ageGroupName + '】图书';
    isHasBookSku.value = 1;
    type.value = 'delete';
  }
  if (data.type == 'add') {
    title.value = '批量添加【' + data.item.ageGroupName + '】图书';
    isHasBookSku.value = 0;
    type.value = 'add';
  }
  if (data.type == 'view') {
    title.value = '查看【' + data.item.ageGroupName + '】图书';
    isHasBookSku.value = 1;
    type.value = 'view';
  }
  ...
  list.value = management({
    ...
    condition: data.type == 'add' ? `id NOT IN (SELECT book_sku_id FROM book_sku_age_group WHERE age_group_id = ${data.item.ageGroupId}) AND language_code = '${data.item.languageCode}'` : `id IN (SELECT book_sku_id FROM book_sku_age_group WHERE age_group_id = ${data.item.ageGroupId}) AND language_code = '${data.item.languageCode}'`,
    pageSize: 8,
    /* getParameters() {
      let params = {}
      params.companyId = companyId.value;
      if (data.type == 'add') {
        params.condition = `id NOT IN (SELECT book_sku_id FROM book_sku_age_group WHERE age_group_id = ${data.item.ageGroupId}) AND language_code = '${data.item.languageCode}'`
      } else {
        params.condition = `id IN (SELECT book_sku_id FROM book_sku_age_group WHERE age_group_id = ${data.item.ageGroupId}) AND language_code = '${data.item.languageCode}'`
      }
      return params;
    }, */
 ...
 * 获取SKU列表
 */
/* function getBookSkuList(bookInfoQuery, bookTagId) {
  let param = {
  ...
  if (bookTagId) {
    param.bookTagId = bookTagId;
  }
  common.call("GetBookSkuListByAgeGroupId", param, (res) => {
...
function deleteSku() {
  executeOperation('DeleteBookSkuAgeGroupByBookSkuIds');
}
/**
 * 添加
 */
function addSku() {
  executeOperation('AddBookSkuAgeGroupByBookSkuIds');
}
/**
 * 执行操作
 */
