...
const init = (options) => {
  ...
  page = detail({
    ...
    getServiceName: 'GetViewHomeRecommend',
    mode: options.mode,
...
const rules = reactive({
  ...
  clubActivityId: [{ required: true, message: '请选择俱乐部活动', trigger: 'change' }],
  displayOrder: [
      ...
      >
        <el-row>
          <el-col :xs="12" :sm="12" :md="12" :lg="12">
            <el-form-item label="俱乐部活动" prop="clubActivityId" label-width="auto">
              <database-select
                placeholder="请选择俱乐部活动"
                table-name="club_activity"
                v-model="page.dataEntity.value.clubActivityId"
                valueFieldName="id"
                @onChange="clubActivityOnChange"
                @onClear="clubActivityOnClear"
