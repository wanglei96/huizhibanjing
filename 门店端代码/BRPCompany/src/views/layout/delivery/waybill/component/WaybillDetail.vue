...
function confirmBtnClick(data){
	let params = {
		id: data.id,
		deliverStatusCode: data.bookOrderTypeCode === 'Borrow'? 'Delivered':'Storageed'
	}
    ...
    <el-card shadow="never" class="user-list-card">
      <el-table
		...
		<el-table-column label="操作" align="center" >
		  <template #default="scope">
			  <el-link type="primary" :id="scope.row.id" @click="confirmBtnClick(scope.row)">
				  <span v-if="scope.row.bookOrderTypeCode === 'Borrow'">确认配送</span>
				  <span  v-else-if="scope.row.bookOrderTypeCode === 'Return'">确认入库</span>
			  </el-link>
		  </template>
		</el-table-column>
      </el-table>
