...
const list = management({
  ...
  getListServiceName: 'GetDelivererTotal',
  refDetail,
...
function exportDataList() {
  ...
  params.exportName = 'DelivererTotal'
  params.pageIndex = 0
...
function getSummaries() {
  ...
  params.companyId = localStorage.getItem('companyId')
  params.isOffline = 0
  params.condition = "book_order_borrow_status_code IN ('Received','ErrorProcessing')"
  params.fields = "ifnull(count(*),0) as order_total"
  common.call('GetViewBookOrderList', params, (res) => {
...
<template>
  <div class="basic-list">
    <el-card shadow="never" class="search-card">
      <el-form ref="refSearchForm" :model="parameters">
        <el-row :gutter="20" type="flex">
          ...
          <el-col :xs="24" :sm="12" :md="8" :lg="6" style="margin-bottom: 10px">
            <el-form-item label="配送员名称" prop="delivererName">
              <el-input placeholder="请输入..." v-model="parameters.delivererName" @keydown.enter.prevent=query></el-input>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="12" :md="8" :lg="6" style="margin-bottom: 10px">
            <el-form-item label="配送路线" prop="deliverLineName">
              <el-input placeholder="请输入..." v-model="parameters.deliverLineName"
    ...
    <el-card shadow="never">
      <template #header>
        <div slot="header" style="color: red;">配送单总数：{{ orderTotal }}</div>
      </template>
      <el-table ref="multipleTableRef" :data="list.dataList.value" style="width: 100%" border
        ...
        <el-table-column property="name" label="配送员" />
