...
const init = (options) => {
  ...
  currentRadio.value = options.data.payOrderId
  list.value = management({
    tableName: 'pay_order',
    orderBy: 'create_time desc',
    getListServiceName: 'GetViewPayOrderList',
    condition: "is_refund is not true",
    pageSize: 5
  })
...
function submit() {
  //获取所有选中的行
  emit('reload-data', { data: currentRow.value });
...
<template>
  <div class="basic-list">
    <el-dialog destroy-on-close :close-on-click-modal="false" v-model="detailFormVisible" title="账单" width="80%"
      draggable>
          ...
          <el-row :gutter="20" type="flex">
            <el-col :xs="24" :sm="12" :md="8" :lg="6">
              <el-form-item label="账单编号" prop="payOrderNo">
                <el-input placeholder="请输入..." v-model="list.parameters.payOrderNo"
            ...
            <el-col :xs="24" :sm="12" :md="8" :lg="6">
              <el-form-item label="账单类型" prop="paymentItemCode">
                <DatabaseSelect table-name="payment_item" v-model="list.parameters.paymentItemCode" placeholder="请选择" />
              </el-form-item>
            </el-col>
            <el-col :xs="24" :sm="12" :md="8" :lg="6">
              <el-form-item>
