            ...
            for (Map.Entry<Integer, List<BookOrderItem>> entry : bookOrderItemMap.entrySet()) {
                ...
                /*for (BookOrderItem item : itemList) {
                    if (item.getBookOrderTypeCode().equals(BookOrderTypeCodes.Borrow)) {
                        ...
                        returnCount++;
                        item.setDeliverStatusCode(DeliverStatusCodes.Storageing);
                        BookOrderItemManager.getInstance().update(sqlSession, item);
                    }
                }*/
                ...
                if (!StringHelper.isNullOrEmpty(deliverTime)) {
                    String deliverTimeStr = deliverTime.substring(0, 10);
                    //获取会员
                    Member theMember = MemberManager.getInstance().getEntity(sqlSession, theBookOrder.getMemberId());
                    ...
                    ViewBookOrder theAreaViewBookOrder = BookOrderManager.getInstance()
                            .getViewBookOrderByArea(sqlSession, companyId, theMember.getDeliverAreaId(), deliverTimeStr);
