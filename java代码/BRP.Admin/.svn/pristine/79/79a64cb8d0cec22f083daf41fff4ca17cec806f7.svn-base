package strosoft.cms.data;
import java.lang.Integer;
import java.sql.Timestamp;
import java.util.List;

import net.sf.json.JSONObject;
import strosoft.app.data.DataEntity;
import java.lang.String;
import strosoft.app.util.JsonHelper;
public class ViewCmsChannelInfo extends DataEntity implements Cloneable
{
    private  Integer id;//;
    private  Integer cmsChannelLanguageId;
    private  String name;//栏目名称;
    private  String title;//标题;
    private  String keyword;//关键字;
    private  String url;//外部链接;
    private  String text;//内容;
    private  String description;//描述;
    private  Integer imageFileId;//图片ID;
    private  String imageFilePath;//图片地址;
    private  String imageFileName;//图片名;
    private  String cmsLanguageCode;//语言编码;
    private  String cmsLanguageName;
    private  String code;//编码;
    private  Integer cmsChannelTemplateId;//栏目模板类型;
    private  Integer cmsContentTemplateId;//内容模板类型;
    private  String cmsChannelTemplateName;//栏目模板名称;
    private  String cmsContentTemplateName;//内容模板名称;
    private  String cmsTemplateUrl;//栏目模板地址;
    private  String cmsContentTypeCode;//内容类型编码;
    private	 Integer cmsContentNum;//内容个数;
    private  Integer cmsSiteId;//所属站点;
    private  String contentFilePathRule;//生成内容地址规则;
    private  String channelFilePathRule;
    private  Timestamp createTime;//时间;
    private  Integer displayOrder;//排序;
    private  Integer parentId;
    private  String parentName;
    private  String createFilePath;//生成页面路径;
    private  List<String> cmsChannelGroupCodeList;//所属栏目编码列表
    private  List<String> cmsChannelGroupNameList;//所属栏目名称列表
    public  void setId(Integer id)    
    {
        this.id = id;
    }
    public  Integer getId()    
    {
        return id;
    }
    public  void setCmsChannelLanguageId(Integer cmsChannelLanguageId)    
    {
        this.cmsChannelLanguageId = cmsChannelLanguageId;
    }
    public  Integer getCmsChannelLanguageId()    
    {
        return cmsChannelLanguageId;
    }
    public  void setName(String name)    
    {
        this.name = name;
    }
    public  String getName()    
    {
        return name;
    }
    public  void setTitle(String title)    
    {
        this.title = title;
    }
    public  String getTitle()    
    {
        return title;
    }
    public  void setKeyword(String keyword)    
    {
        this.keyword = keyword;
    }
    public  String getKeyword()    
    {
        return keyword;
    }
    public  void setUrl(String url)    
    {
        this.url = url;
    }
    public  String getUrl()    
    {
        return url;
    }
    public  void setText(String text)    
    {
        this.text = text;
    }
    public  String getText()    
    {
        return text;
    }
    public  void setDescription(String description)    
    {
        this.description = description;
    }
    public  String getDescription()    
    {
        return description;
    }
    public  void setImageFileId(Integer imageFileId)    
    {
        this.imageFileId = imageFileId;
    }
    public  Integer getImageFileId()    
    {
        return imageFileId;
    }
    public  void setImageFilePath(String imageFilePath)    
    {
        this.imageFilePath = imageFilePath;
    }
    public  String getImageFilePath()    
    {
        return imageFilePath;
    }
    public  void setImageFileName(String imageFileName)    
    {
        this.imageFileName = imageFileName;
    }
    public  String getImageFileName()    
    {
        return imageFileName;
    }
    public  void setCmsLanguageCode(String cmsLanguageCode)    
    {
        this.cmsLanguageCode = cmsLanguageCode;
    }
    public  String getCmsLanguageCode()    
    {
        return cmsLanguageCode;
    }
    public  void setCmsLanguageName(String cmsLanguageName)    
    {
        this.cmsLanguageName = cmsLanguageName;
    }
    public  String getCmsLanguageName()    
    {
        return cmsLanguageName;
    }
    public  void setCode(String code)    
    {
        this.code = code;
    }
    public  String getCode()    
    {
        return code;
    }
    public  void setCmsChannelTemplateId(Integer cmsChannelTemplateId)    
    {
        this.cmsChannelTemplateId = cmsChannelTemplateId;
    }
    public  Integer getCmsChannelTemplateId()    
    {
        return cmsChannelTemplateId;
    }
    public  void setCmsContentTemplateId(Integer cmsContentTemplateId)    
    {
        this.cmsContentTemplateId = cmsContentTemplateId;
    }
    public  Integer getCmsContentTemplateId()    
    {
        return cmsContentTemplateId;
    }
    public  void setCmsChannelTemplateName(String cmsChannelTemplateName)    
    {
        this.cmsChannelTemplateName = cmsChannelTemplateName;
    }
    public  String getCmsChannelTemplateName()    
    {
        return cmsChannelTemplateName;
    }
    public  void setCmsContentTemplateName(String cmsContentTemplateName)    
    {
        this.cmsContentTemplateName = cmsContentTemplateName;
    }
    public  String getCmsContentTemplateName()    
    {
        return cmsContentTemplateName;
    }
    public  void setCmsTemplateUrl(String cmsTemplateUrl)    
    {
        this.cmsTemplateUrl = cmsTemplateUrl;
    }
    public  String getCmsTemplateUrl()    
    {
        return cmsTemplateUrl;
    }
    public  void setCmsContentTypeCode(String cmsContentTypeCode)    
    {
        this.cmsContentTypeCode = cmsContentTypeCode;
    }
    public  String getCmsContentTypeCode()    
    {
        return cmsContentTypeCode;
    }
    public  void setCmsSiteId(Integer cmsSiteId)    
    {
        this.cmsSiteId = cmsSiteId;
    }
    public  Integer getCmsSiteId()    
    {
        return cmsSiteId;
    }
    public  void setContentFilePathRule(String contentFilePathRule)    
    {
        this.contentFilePathRule = contentFilePathRule;
    }
    public  String getContentFilePathRule()    
    {
        return contentFilePathRule;
    }
    public  void setChannelFilePathRule(String channelFilePathRule)    
    {
        this.channelFilePathRule = channelFilePathRule;
    }
    public  String getChannelFilePathRule()    
    {
        return channelFilePathRule;
    }
    public  void setCreateTime(Timestamp createTime)    
    {
        this.createTime = createTime;
    }
    public  Timestamp getCreateTime()    
    {
        return createTime;
    }
    public  void setDisplayOrder(Integer displayOrder)    
    {
        this.displayOrder = displayOrder;
    }
    public  Integer getDisplayOrder()    
    {
        return displayOrder;
    }
    public  void setParentId(Integer parentId)    
    {
        this.parentId = parentId;
    }
    public  Integer getParentId()    
    {
        return parentId;
    }
    public  void setCreateFilePath(String createFilePath)    
    {
        this.createFilePath = createFilePath;
    }
    public  String getCreateFilePath()    
    {
        return createFilePath;
    }
    public  ViewCmsChannelInfo copy()    
    {
        ViewCmsChannelInfo entity = null;
        try        
        {
            entity = (ViewCmsChannelInfo) super.clone();
        }
         catch (CloneNotSupportedException e)         
        {
            e.printStackTrace();
        }
        return entity;
    }
    public  JSONObject toJson()    
    {
        return JSONObject.fromObject(this,JsonHelper.getJsonConfig());
    }
	public Integer getCmsContentNum() {
		return cmsContentNum;
	}
	public void setCmsContentNum(Integer cmsContentNum) {
		this.cmsContentNum = cmsContentNum;
	}
	public List<String> getCmsChannelGroupCodeList() {
		return cmsChannelGroupCodeList;
	}
	public void setCmsChannelGroupCodeList(List<String> cmsChannelGroupCodeList) {
		this.cmsChannelGroupCodeList = cmsChannelGroupCodeList;
	}
	public List<String> getCmsChannelGroupNameList() {
		return cmsChannelGroupNameList;
	}
	public void setCmsChannelGroupNameList(List<String> cmsChannelGroupNameList) {
		this.cmsChannelGroupNameList = cmsChannelGroupNameList;
	}
	public String getParentName() {
		return parentName;
	}
	public void setParentName(String parentName) {
		this.parentName = parentName;
	}
}