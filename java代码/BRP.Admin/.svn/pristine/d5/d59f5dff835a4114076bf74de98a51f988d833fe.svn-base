...
public class EditMemberInfoServiceHandler extends ServiceHandler {
    ...
    public void process(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // 校验当前手机号是否存在
            Member memberCheck = MemberManager.getInstance().getEntity(sqlSession, memberId);
            // 判断当前会员是否修改了手机号，查询修改后的手机号是否存在
            if (!mobilePhone.equals(memberCheck.getMobilePhone())) {
                Integer count = MemberManager.getInstance().getCount(sqlSession, "mobile_phone = " + mobilePhone);
                if (count > 0) {
            ...
            if (!StringUtils.isEmpty(effectiveDateText) && !StringUtils.isEmpty(expireDateText)) {
                // 如果修改了开卡时间生成一条记录
                if (memberCard.getExpireDate() != null && !memberCard.getExpireDate().equals(DateHelper.toTimestamp(expireDateText))) {
                    // 会员卡变更记录
                    ...
                    memberCardServiceTimeLog.setExpiryTime(DateHelper.toTimestamp(expireDateText));
                    memberCardServiceTimeLog.setAgoExpiryTime(memberCard.getExpireDate());
                    ...
                    if (memberCard.getExpireDate() != null) {
                        ...
                        LocalDateTime expireDateTime = LocalDateTime.parse(expireDateText, formatter);

                        // 将 Timestamp 转换为 LocalDateTime
                        LocalDateTime timestampDateTime = memberCard.getExpireDate().toInstant()
                                .atZone(ZoneId.systemDefault())
                                .toLocalDateTime();

                        // 计算天数差并转换为 int 类型
                        long daysDifferenceLong = ChronoUnit.DAYS.between(timestampDateTime.toLocalDate(), expireDateTime.toLocalDate());

                        memberCardServiceTimeLog.setDays((int) daysDifferenceLong);
                    }
