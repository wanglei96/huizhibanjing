    ...
    public void process(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!hasReturnItem) {
                ...
                //修改配送单还书状态为待消毒
                if(!bookOrder.getBookOrderReturnStatusCode().equals(BookOrderReturnStatusCodes.Finished)){
                    bookOrder.setBookOrderReturnStatusCode(BookOrderReturnStatusCodes.Sterilizing);
                }
            }
            if(!StringHelper.isNullOrEmpty(errorRemark)){
            ...
            for (BookOrderItem bookOrderItem : bookOrderItemList) {
                ...
                if (bookOrderItem.getBookOrderTypeCode().equals("Return")) {
                    /*if(!bookOrderItem.getDeliverStatusCode().equals("Storageing")){
            ...
            if (!StringHelper.isNullOrEmpty(theViewBookOrder.getWxaOpenId())) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        String result = MessageManager.getInstance().sendDeliveryMessage(theViewBookOrder, lstViewBookOrderItem, bookNames);
                    } catch (Exception e) {
                        // 处理发送消息失败的情况
                        e.printStackTrace();
                    }
