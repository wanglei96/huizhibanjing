...
public class GetBookpickerTotalServiceHandler extends GetListServiceHandler {
    public void process(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String sqlCount = String.format("SELECT COUNT(*) AS total_records\n" +
                ...
                "    LEFT JOIN\n" +
                "        deliver_line dl ON cu.id = dl.bookpicker_id\n" +
                "    WHERE cu.is_bookpicker = 1 AND cu.company_id = %d\n" +
                "    GROUP BY cu.id, cu.name, cu.company_id, dl.name\n" +
