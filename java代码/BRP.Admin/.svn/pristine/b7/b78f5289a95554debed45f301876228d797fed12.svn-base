...
public class GetViewBookSkuInfoListServiceHandler extends GetListServiceHandler {
    public void process(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (bookSkuId != null) {
            conditionBuilder.add("book_sku.id !=" + bookSkuId);
        }
        ...
        if (bookSkuId != null) {
            conditionBuilder.add("id !=" + bookSkuId);
        }
        ...
        if (!StringHelper.isNullOrEmpty(available)) {
            conditionCountBuilder.add(String.format(
                    "( SELECT COUNT(*) FROM book WHERE book.book_sku_id = view_book_sku.id AND book.is_available = 1  AND book.book_status_code='On') %s 0", available));
        }
