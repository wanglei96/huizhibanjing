...
public class UploadFileServiceHandler extends ServiceHandler {
    ...
    private void uploadNormalFile(HttpServletRequest request, HttpServletResponse response, FileItem theFileItem) throws Exception {
        try {
            ...
            if (isImage) {
                ...
                this.processSmallImage(request, newSysUploadFile, fileName, theFileItem);
            }
     ...
     */
    private void processSmallImage(HttpServletRequest request, SysUploadFile newSysUploadFile, String fileName, FileItem theFileItem)
            throws Exception {
        ...
        String sourceFilePath = SysConfig.getResourceDirectoryPath() + "/" + newSysUploadFile.getPath();
        //处理旋转图像
        BufferedImage originalImage = processImage(theFileItem, sourceFilePath);
        File originalFile = new File(sourceFilePath);
        if (originalFile.exists()) {
            originalFile.delete(); // 删除旧文件
        }
        // 将处理后的图像保存到原始文件路径
        ImageIO.write(originalImage, "jpg", new File(sourceFilePath));
    ...
    public static BufferedImage processImage(FileItem fileItem, String outputPath) {
        BufferedImage image = null; // 用于返回的 BufferedImage
        try {
            // 获取输入流并读取图像
            ...
            // 读取EXIF信息
            Metadata metadata = ImageMetadataReader.readMetadata(fileItem.getInputStream());
    ...
    private static BufferedImage rotateImage(BufferedImage image, int degrees) {
        // 创建一个新的 BufferedImage 来保存旋转后的图像
        int newWidth = degrees == 90 || degrees == 270 ? image.getHeight() : image.getWidth();
        int newHeight = degrees == 90 || degrees == 270 ? image.getWidth() : image.getHeight();

        // 新建图像，并保持原图像的类型
        BufferedImage rotatedImage = new BufferedImage(newWidth, newHeight, image.getType());
        Graphics2D g2d = rotatedImage.createGraphics();

        // 设置图形上下文的抗锯齿属性，提升图像质量
        ...
        // 旋转并绘制图像
        // 先平移图像到原点，然后进行旋转，最后平移回去
        g2d.translate(newWidth / 2, newHeight / 2); // 平移到新图像中心
        g2d.rotate(Math.toRadians(degrees)); // 旋转
        g2d.translate(-centerX, -centerY); // 平移回原位置
        g2d.drawImage(image, 0, 0, null); // 绘制图像
        g2d.dispose(); // 释放资源

        return rotatedImage; // 返回旋转后的新图像
    }
}
