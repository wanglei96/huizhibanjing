 ...
 * 取得用户菜单列表
 */
public class GetFrontMenuListByUserIdServiceHandler extends ServiceHandler {
	...
	public void process(HttpServletRequest request, HttpServletResponse response)
		...
		Hashtable<Integer, SysMenu> sysMenuHashtable = SysMenuManager
				.getInstance().getSysMenuHashtable(allSysMenu);
		// 获取查询的菜单列表
		...
		for (int i = 0; i < lstSysMenu.size() - 1; i++) {
			for (int j = i + 1; j < lstSysMenu.size(); j++) {
				if (lstSysMenu.get(i).getDisplayOrder() > lstSysMenu.get(j)
						.getDisplayOrder()) {
