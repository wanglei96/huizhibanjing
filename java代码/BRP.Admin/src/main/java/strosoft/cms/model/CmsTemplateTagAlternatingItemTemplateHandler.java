...
public class CmsTemplateTagAlternatingItemTemplateHandler extends CmsTemplateTagHandler {
	...
	public String process(CmsHtmlGenerateContext context,CmsBuildContext cmsBuildContext) {
		...
		String html = "";
		try {
			//判断是否是奇数行,不是奇数行返回空字符串
			if(null == (Integer)context.getNumOnPage() || context.getNumOnPage()%2 == 0){
				return "";
			}
			//生成子节点
