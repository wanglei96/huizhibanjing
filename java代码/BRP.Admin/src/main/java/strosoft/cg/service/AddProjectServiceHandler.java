 ...
 * 添加服务处理类
 */
public class AddProjectServiceHandler extends ServiceHandler {
	...
	public void process(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			...
			String configuration = CreateCodeManager.getInstance().getConfiguration(project).replace("\\", "\\\\");
			System.out.println(configuration);
			boolean isXMLFormat = CreateCodeManager.getInstance().checkXMLFormat(configuration);
			if(isXMLFormat){
				//将数据插入数据库
				String sql = "insert into sys_build_project (name,configuration,create_time) values ('"+project.getProjectName()+"','"+configuration+"',now())";
