...
public class UpdateBookOrderInfoServiceHandler extends UpdateServiceHandler {
    ...
    protected DataEntity processDataEntity(ServiceContext updateServiceContext, DataEntity newDataEntity)
        ...
        if (newBookOrder.getDeliverTime() != null) {
            String deliverTimeStr = DateHelper.timestampToString(newBookOrder.getDeliverTime(), "yyyy-MM-dd");
            //获取大区排序
            ViewBookOrder theAreaViewBookOrder = BookOrderManager.getInstance()
                    .getViewBookOrderByArea(sqlSession, newBookOrder.getCompanyId(), theViewMemberCard.getDeliverAreaId(), deliverTimeStr);
            if (theAreaViewBookOrder == null) {
                newBookOrder.setDeliverAreaNumber(1);
            } else {
                newBookOrder.setDeliverAreaNumber(theAreaViewBookOrder.getDeliverAreaNumber() + 1);
            }
        }
        ...
        for (BookOrderItem bookOrderItem : bookOrderItemList) {
            ...
            if (bookOrderItem.getBookOrderTypeCode().equals(BookOrderTypeCodes.Borrow)) {
                ...
                if (bookOrderItem.getId() != null) {
                    ...
                    bookOrderItem.setDeliverStatusCode(DeliverStatusCodes.Storageing);
                    bookOrderItem.setBookOrderId(theBookOrder.getId());
                    ...
                    String condition = String.format(
                            ...
                            bookOrderItem.getBookId(), theBookOrder.getMemberId(), BookOrderTypeCodes.Borrow);
                    List<ViewBookOrderItem> borrowBookOrderItemList = ViewBookOrderItemManager.
                            getInstance().getList(sqlSession, condition, "create_time desc");
