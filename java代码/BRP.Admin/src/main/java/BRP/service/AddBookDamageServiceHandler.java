...
public class AddBookDamageServiceHandler extends AddServiceHandler {
                ...
                if (memberId != null) {
                    ...
                    if (borrowBookOrderItemList.size() > 0) {
                        ...
                        String conditionStorageing = String.format(
                                "book_id= %d and member_id= %d and deliver_status_code='%s' and book_order_type_code='%s'",
                                theBook.getId(), memberId, DeliverStatusCodes.Storageing, BookOrderTypeCodes.Return);
                        ...
                        if(lstViewBookOrderItem.size() > 0){
                            ViewBookOrderItem theViewBookOrderItem = lstViewBookOrderItem.get(0);
                            int newBookOrderId = theViewBookOrderItem.getBookOrderId();
                            int newBookOrderItemId = theViewBookOrderItem.getId();
                            //修改状态
                            String sql = String.format("update book_order_item set deliver_status_code = '%s',is_returned = 1 where id = %d",
                                    DeliverStatusCodes.Storageed, newBookOrderItemId);
                            MyBatisManager.getInstance().executeUpdate(session, sql);
                            //修改单据还书状态为完成
                            String bookOrderCondition = String.format("book_order_id = %d and book_order_type_code = '%s'",
                                    newBookOrderId, BookOrderTypeCodes.Return);
                            ...
                            BookOrder theBookOrder = BookOrderManager.getInstance().getEntity(session, newBookOrderId);
                            if (allDelivered) {
    ...
    private void addBookInOrder(SqlSession session, int newBookOrderItemId, Book book) throws IOException {
        //添加入库单
