...
public class ViewMemberCardManager extends ViewMemberCardMaster {
    ...
    private BorrowReturnBookInfo getListByMemmberInfo(Integer memberCartId) throws Exception {
        String sql = String.format("SELECT *,\n" +
                "                       (SELECT COUNT(1) FROM book WHERE book.member_id = view_member_card_simple.member_id AND borrow_status_code = 'Out') AS borrowing_book_count,\n" +
                "                       (SELECT COUNT(1) FROM view_book_order_item WHERE view_book_order_item.member_id = view_member_card_simple.member_id AND book_order_type_code = 'Borrow') AS borrowed_book_count,\n" +
                "                       IFNULL((SELECT COUNT(1) FROM book_order WHERE book_order.member_id = view_member_card_simple.member_id AND book_order.id IN (SELECT book_order_id FROM book_order_item WHERE book_order_type_code = 'Borrow' AND book_order.id = book_order_item.book_order_id)), 0) AS borrow_count\n" +
                "                FROM view_member_card_simple \n" +
        ...
        for (int i = 0; i <= sheet.getLastRowNum(); i++) {
            ...
            if (newMemberCardCardNoStr.isEmpty() || referrerCardCardNoStr.isEmpty()) {
                ...
                if (newMemberCardCard == null || referrerCardCard == null) {
                    ...
                    recommendMemberCardInfo.setNewMemberCardExpireDateText(DateHelper.timestampToString(newMemberCardCard.getExpireDate(), "yyyy-MM-dd HH:mm:ss"));
                    recommendMemberCardInfo.setNewExpireDate(newMemberCardCard.getExpireDate());
                    recommendMemberCardInfo.setNewExpireDateText(DateHelper.timestampToString(newMemberCardCard.getExpireDate(), "yyyy-MM-dd HH:mm:ss"));
                    ...
                    recommendMemberCardInfo.setReferrerCardEffectiveDateText(DateHelper.timestampToString(referrerCardCard.getEffectiveDate(), "yyyy-MM-dd HH:mm:ss"));
                    recommendMemberCardInfo.setReferrerCardExpireDate(referrerCardCard.getExpireDate());
                    recommendMemberCardInfo.setReferrerCardExpireDateText(DateHelper.timestampToString(referrerCardCard.getExpireDate(), "yyyy-MM-dd HH:mm:ss"));
                    recommendMemberCardInfo.setReferrerExpireDate(referrerCardCard.getExpireDate());
                    recommendMemberCardInfo.setReferrerExpireDateText(DateHelper.timestampToString(referrerCardCard.getExpireDate(), "yyyy-MM-dd HH:mm:ss"));
