    ...
    public ViewBookInOrder addBookInOrder(Integer companyId, String bookSn, Boolean isEnabled) throws Exception {
        ...
        try {
            ...
            if (lstBookOrderItem.size() == 0) {
                throw new Exception("请核对图书编号，该图书编号未找到待入库记录！");
            }
            ...
            //获取会员信息
            if (theBookOrder.getMemberId() != null) {
                //获取会员卡
                MemberCard memberCard = MemberCardManager.getInstance().getEntityByMemberId(sqlSession, theBookOrder.getMemberId() );
                if (memberCard != null) {
                    if ( !StringHelper.isNullOrEmpty(memberCard.getMemberCardStatusCode()) && memberCard.getMemberCardStatusCode().equals("Locked")) {
                        //获取会员等待入库图书数量
                        String storageingCondition = String.format("member_id = %d and book_order_type_code='%s' and deliver_status_code='%s'"
                                , theBookOrder.getMemberId() , BookOrderTypeCodes.Return, DeliverStatusCodes.Storageing);
