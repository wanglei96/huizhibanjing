...
public class ExportMemberHandler extends ExportHandler {
    ...
    protected Object getData(ExportContext context) throws Exception {
        ...
        String memberInfoCardStatusQuery = JsonHelper.getString(jData, "memberInfoCardStatusQuery");
        ...
        String memberCardIds = JsonHelper.getString(jData, "memberCardIds");
        ...
        if (!StringHelper.isNullOrEmpty(memberCardIds)) {
            conditionBuilder.add("view_member_card.id in (" + memberCardIds + ")");
        }
        if (!StringHelper.isNullOrEmpty(isBook)) {
            if (isBook.equals("NoBookbox")) {
                conditionBuilder.add("view_member_card.bookbox_amount is null");
            } else if (isBook.equals("WaitReady")) {
                conditionBuilder.add("view_member_card.member_bookbox_status_code='WaitReady' and view_member_card.bookbox_amount is not null");
        ...
        if (isExpire != null) {
            if (isExpire == 0) {
                conditionBuilder.add("view_member_card.expire_date >= CURDATE()");
            } else if (isExpire == 1) {
                conditionBuilder.add("view_member_card.expire_date < CURDATE()");
            } else if (isExpire == 2) {
                conditionBuilder.add("DATEDIFF(view_member_card.expire_date, CURDATE()) <= 30");
            } else if (isExpire == 3) {
                conditionBuilder.add("DATEDIFF(view_member_card.expire_date, CURDATE()) <= 60");
            } else if (isExpire == 4) {
                conditionBuilder.add("DATEDIFF(view_member_card.expire_date, CURDATE()) <= 90");
            }
        }

        if (!StringHelper.isNullOrEmpty(memberInfoCardStatusQuery)) {
            if (memberInfoCardStatusQuery.equals("StopCardIng")) {
                conditionBuilder.add("view_member_card.is_stop_card is true");
            } else if (memberInfoCardStatusQuery.equals("Locked")) {
                conditionBuilder.add("view_member_card.member_card_status_code='Locked'");
            } else if (memberInfoCardStatusQuery.equals("Normal")) {
                conditionBuilder.add("view_member_card.member_card_status_code='Normal' and view_member_card.is_stop_card is not true");
            }
        }
        ...
        String sql = String.format("SELECT\n" +
                ...
                "\t\tCASE\n" +
                "\t\t\t\t\n" +
                "\t\t\t\tWHEN ( view_member_card.is_stop_card IS not TRUE ) THEN\n" +
                "\t\t\t\t'否' ELSE '是' \n" +
                "\t\t\tEND \n" +
                "\t\t\t) AS `stop_card_name`,\n" +
                ...
                "\t\t\tAND member_card_disable_log.type ='decrease' \n" +
