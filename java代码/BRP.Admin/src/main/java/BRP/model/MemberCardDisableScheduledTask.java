        ...
        try {
            ...
            if (lstLog.size() > 0) {
                ...
                for (MemberCardDisableLog log : lstLog) {
                    ...
                    //获取减去天数的时间
                    LocalDateTime localDateTime = theMemberCard.getExpireDate().toLocalDateTime();

                    LocalDateTime newLocalDateTime = localDateTime.minus(log.getDays(), ChronoUnit.DAYS);

                    Timestamp agoExpiryTime = Timestamp.valueOf(newLocalDateTime);

                    //添加会员时长变更记录
                    MemberCardServiceTimeLog newMemberCardServiceTimeLog = new MemberCardServiceTimeLog();
                    newMemberCardServiceTimeLog.setMemberId(theMember.getId());
                    newMemberCardServiceTimeLog.setDays(log.getDays());
                    ...
                    newMemberCardServiceTimeLog.setAgoExpiryTime(agoExpiryTime);
                ...
                if (lstMemberCardServiceTimeLog.size() > 0) {
                    MemberCardServiceTimeLogManager.getInstance().addList(session, lstMemberCardServiceTimeLog);
                }
            }
